...
import { formatStakingAmount, StakingTransaction, StakeInfo, getTierByLockDays } from '@/lib/staking-api'
...
export function StakingHistory({ stakes, history, onClose }: StakingHistoryProps) {
  ...
  const getIcon = (type: string) => {
    switch (type) {
      case 'stake':
        return <ArrowDownRight className="w-4 h-4 text-gold-200" />
      case 'unstake':
  ...
  return (
    <div className="fixed inset-0 bg-vault-base/95 flex items-center justify-center z-50 p-4">
      <div className="bg-vault-card rounded-2xl max-w-lg w-full border border-vault-border max-h-[85vh] flex flex-col">
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
          ...
          {stakes.length > 0 && stakes.some(s => Number(s.amount) > 0) && (
            <div>
              <h3 className="text-sm font-medium text-gray-500 mb-3">Active Stakes</h3>
              <div className="space-y-2">
                {stakes.filter(s => Number(s.amount) > 0).map((stake, i) => {
                  ...
                  return (
                    <div
                      key={i}
                      className="bg-vault-base/50 rounded-xl p-4 border border-vault-border"
                    >
          ...
          {/* Transaction History */}
          <div>
            <h3 className="text-sm font-medium text-gray-500 mb-3">Transaction History</h3>

            {history.length === 0 ? (
              <div className="text-center py-8 text-gray-500">
                No staking history yet. Start by staking some STTAURX tokens!
              </div>
            ) : (
              <div className="space-y-4">
                {sortedDates.map(date => (
                  <div key={date}>
                    <div className="text-xs text-gray-500 mb-2 sticky top-0 bg-vault-card py-1">
                      {date}
                    </div>
                    <div className="space-y-2">
                      {groupedHistory[date].reverse().map((tx, i) => (
                        <div
                          key={i}
                          className="bg-vault-base/30 rounded-lg p-3 flex items-center justify-between border border-vault-border"
                        >
                          <div className="flex items-center gap-3">
                            <div className="p-2 bg-vault-cardHover rounded-lg">
                              {getIcon(tx.type)}
                            </div>
                            <div>
                          ...
                          <div className="text-right">
                            <p className={`font-medium ${getAmountColor(tx.type)}`}>
                              {tx.type === 'unstake' ? '-' : '+'}
                              {formatStakingAmount(tx.amount)} STTAURX
                            </p>
                            {tx.yieldRate && (
                              <p className="text-xs text-gray-500">
        ...
        <div className="p-4 border-t border-vault-border">
          <button
            onClick={onClose}
            className="w-full py-3 bg-vault-cardHover hover:bg-gray-700 border border-vault-border rounded-xl font-medium transition-colors"
          >
            Close
          </button>
        </div>
